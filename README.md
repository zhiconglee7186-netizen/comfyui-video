# ComfyUI GUI Script Guide (`comfyui_gui.py`)
...
## Quick Start
1. Start ComfyUI server and confirm API is reachable.
2. Ensure `ffmpeg`/`ffprobe` are in `PATH`.
3. Put workflow JSON files next to `comfyui_gui.py` (or in a resource path used by your packaged build).
4. Run:
